...
import uesDeleteBanner from "@/database/delete/uesDeleteBanner";
import uesAllBanner from "@/database/find/allBanner/useAllBanner";
...
export default function ManageBanner() {
  ...
  const BannerLoad = async () => {
    setLoading(true);
    const { allBanner } = await uesAllBanner();
    setBannerInfo(allBanner);
    setLoading(false);
  };
  const deleteMovie = async (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        const res = uesDeleteBanner(id);
        Swal.fire({
          title: "Deleted!",
          text: "banner deleted success",
          icon: "success",
        });
      }
    });
              ...
              {BannerInfo.map((m, index) => (
                <tr key={m?._id}>
                  ...
                  <td className="capitalize">
                    <Link
                      exact="true"
                      href={`/dashboard/manage-banner/update/${m.catagories}/${m?._id}`}
                    >
